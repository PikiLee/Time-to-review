...
watchEffect(() => {
	dayjs.locale(dayjsLocales[i18n.global.locale.value as keyof typeof dayjsLocales])
...
export function useCreatedTime(raw: MaybeComputedRef<string | number | Date>) {
	...
	watch(localeChangedIndicator, () => createdTime.value = getCreatedTime(), {immediate: true})
